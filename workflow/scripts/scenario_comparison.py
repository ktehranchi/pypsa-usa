...
class ScenarioDataGetter:
    ...
    def _custom_statistics(self, n: pypsa.Network) -> pd.DataFrame:
        ...
        for c in n.generators.carrier.unique():
            for p in horizons:
                capacity.loc[("Generator", c), p] = n.generators[
                    n.generators.carrier == c
                ].p_nom_opt.sum()
        ...
        if not n.storage_units.empty:
            for c in n.storage_units.carrier.unique():
                for p in horizons:
                    capacity.loc[("StorageUnit", c), p] = n.storage_units[
            ...
            try:
                ...
                if as_pct:
                    ...
                    if (total > 0).all():
                        ...
                        logger.warning(f"Zero sum detected in {scenario} for {variable}, skipping percentage conversion")
    ...
    def export_summary_table(self, variables: List[str]) -> None:
        ...
        summary_df.to_csv(self.figures_path / "summary_table.csv", index=False)
        
        # Also export as formatted Excel
        try:
            summary_df.to_excel(self.figures_path / "summary_table.xlsx", index=False)
        except ImportError:
...
class ScenarioPlotter:
    ...
    def plot_scenario_comparison(
        ...
        legend_handles = [
            plt.Rectangle((0, 0), 1, 1, color=self.colors.get(tech, "#999999"))
            for tech in carriers_plotted.index
        ]
        
        # Get legend name or use index if not available
        legend_labels = [
            carriers_plotted.get("legend_name", {}).get(tech, tech) 
            if hasattr(carriers_plotted, "get") else tech
            for tech in carriers_plotted.index
        ]
    ...
    def _plot_reference_comparison(
        ...
        plt.ylabel("∆ Capacity[%]")
        output_file = self.figures_path / f"{variable}_pct_comparison{filename_suffix}.{plot_format}"
        plt.savefig(
            output_file,
            dpi=300,
            bbox_inches="tight",
